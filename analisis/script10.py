    ...
    for file in files:
        ...
        with open(file, 'r', encoding='latin-1') as f:
            for line in f:
                ...
                if len(parts) == 5: 
                    stock_name, open_price, high_price, low_price= parts[:4]
                    df = pd.DataFrame([[stock_name, float(open_price), float(high_price), float(low_price),date]], columns=['Stock', 'Open', 'High', 'Low', 'Date'])
                    dfs.append(df)
...
if __name__ == "__main__":
    ...
    command = ['python', '10.py', temp_csv_file, base_path + 'company_info.csv' ]
